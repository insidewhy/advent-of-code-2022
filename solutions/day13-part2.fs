...
let divider1 = NumList [ NumList [ Num 2 ] ]
let divider2 = NumList [ NumList [ Num 6 ] ]

type DividerPositions = { Before: int; Between: int }

let buildNextDividerPositions (acc: DividerPositions) (next: RecursiveNumList): DividerPositions =
  match (compareSides next divider1) with
    | Lt -> { acc with Before = acc.Before + 1 }
    | _ ->
      match (compareSides next divider2) with
      | Lt -> { acc with Between = acc.Between + 1 }
      | _ -> acc

let dividers = Seq.fold buildNextDividerPositions { Before = 0; Between = 0 } parsedLines
printfn "%d" ((dividers.Before + 1) * (dividers.Before + dividers.Between + 2))
